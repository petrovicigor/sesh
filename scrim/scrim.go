...
// keep is how much of a color survives the blend toward the scrim base —
// the single dimming knob. 0.35 matches the OpenCode reference: content
// stays recognizable but unmistakably behind the panel.
const keepNum, keepDen = 35, 100

type rgb struct{ r, g, b uint8 }

// mix blends c toward base, keeping keepNum/keepDen of c's distance from it.
func mix(c, base rgb) rgb {
	ch := func(c, b uint8) uint8 {
		return uint8(int(b) + (int(c)-int(b))*keepNum/keepDen)
	}
	return rgb{ch(c.r, base.r), ch(c.g, base.g), ch(c.b, base.b)}
}
...
//
// The base sits BEYOND the theme background (darker in both modes — the
// universal modal convention) so the panel, which keeps the default
// background, pops without a border.
...
// snapshot renders. Default backgrounds land exactly on the scrim base;
// everything else blends toward it. Bold survives (dimmed content keeps its
// texture); reverse is resolved here by swapping.
func dimStyle(st style, p *palette) style {
	fgRef, bgRef := st.fg, st.bg
	if st.reverse {
		fgRef, bgRef = bgRef, fgRef
	}
	fg := mix(resolve(fgRef, !st.reverse, p), p.base)
	var bg rgb
	if bgRef.kind == colDefault && !st.reverse {
		bg = p.base
	} else {
		bg = mix(resolve(bgRef, st.reverse, p), p.base)
	}
	return style{
